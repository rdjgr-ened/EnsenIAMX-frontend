...
export default function HojaDeTrabajoModal(props: HojaDeTrabajoModalProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 pb-16 animate-fade-in print:bg-white print:pb-0">
      
      {/* HEADER DE LA PÁGINA (Actúa como barra de navegación) */}
      <div className="bg-slate-900 text-white p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-md print:hidden">
        ...
        <button
          type="button"
          onClick={() => safeOnClose()}
          className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl transition cursor-pointer font-bold text-sm flex items-center gap-2"
        >
          <X className="w-5 h-5" />
      ...
      {/* CONTENIDO PRINCIPAL (Flujo natural del documento) */}
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 mt-8 print:max-w-none print:px-0 print:mt-0">
        ...
        {error && (
          <div className="p-6 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 space-y-3 mt-8">
            ...
            <button
              type="button"
              onClick={() => safeOnRetry()}
              className="px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white font-bold text-sm uppercase tracking-wider rounded-xl transition cursor-pointer"
            >
        ...
        {worksheet && !isLoading && (
          <div className="space-y-6">
            ...
            <div id="hoja-trabajo-resultado" className="bg-white p-6 sm:p-10 rounded-2xl border border-slate-300 shadow-sm space-y-8 text-slate-900 text-xs printable-document print:border-none print:shadow-none print:p-0">
              ...
              <div className="border-2 border-slate-900 rounded-xl p-5 bg-slate-50/50 space-y-4">
                <div className="text-center">
                  ...
                  <h2 className="font-bold text-slate-700 text-sm mt-1">
              ...
              <div className="bg-amber-50/60 border border-amber-200 p-4 rounded-xl text-slate-800 text-sm font-medium">
                <span className="font-black uppercase text-amber-900 block text-xs tracking-wider mb-1">
                  📌 Instrucciones Generales:
                </span>
              ...
              <div className="space-y-4">
                <h3 className="font-black text-sm uppercase text-mex-maroon tracking-wider border-b-2 border-mex-maroon pb-1">
                  {worksheet.seccionInicio.titulo}
                </h3>
                <p className="text-slate-600 font-semibold text-xs italic">
              ...
              {worksheet.ticketDeSalida && (
                <div className="p-5 border-2 border-dashed border-amber-500 bg-amber-50/40 rounded-xl space-y-3 break-inside-avoid">
                  <span className="font-black uppercase text-amber-900 text-sm tracking-wider flex items-center gap-1.5">
